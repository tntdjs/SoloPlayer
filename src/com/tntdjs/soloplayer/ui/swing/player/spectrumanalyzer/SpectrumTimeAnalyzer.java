 ...
 *----------------------------------------------------------------------
 */
package com.tntdjs.soloplayer.ui.swing.player.spectrumanalyzer;
...
public class SpectrumTimeAnalyzer extends JPanel implements KJDigitalSignalProcessor {
	...
	private static final long serialVersionUID = 6490562814290660356L;
	private static Logger LOGGER = LogManager.getLogger(SpectrumTimeAnalyzer.class.getName());
//	private static Log log = LogFactory.getLog(SpectrumTimeAnalyzer.class);
	...
	public void setVisColor(String viscolor) {
		ArrayList<Color> visColors = new ArrayList<Color>();
	...
	private void drawVUMeter(Graphics pGrp, float[] pLeft, float[] pRight, float pFrrh) {
		...
		// drawVolumeMeterBar( pGrp, 16, wHeight + 22, (int)( ( vuAverage /
		// vuSamples ) * (float)( width - 32 ) ), 4 );
		drawVolumeMeterBar(pGrp, 16, wHeight + 32, (int) (oldRight * (float) (width - 32)), wHeight);
		// pGrp.fillRect( 16, 16, (int)( oldLeft * (float)( width - 32 ) ),
		// wHeight );
		// pGrp.fillRect( 16, 64, (int)( oldRight * (float)( width - 32 ) ),
		// wHeight );
	}
	 ...
	 *
	 * @param pMode
	 *            Must be either DISPLAY_MODE_SCOPE or
	 *            DISPLAY_MODE_SPECTRUM_ANALYSER.
	 */
	...
	/*
	 * public void update(Graphics pGraphics) { // -- Prevent AWT from clearing
	 * background. paint(pGraphics); }
	 */
}
